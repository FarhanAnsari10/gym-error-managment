...
export const uploadFileToCloudinary = async (
  ...
  try {
    ...
    if (file && file.uri) {
      ...
      if (!response.ok) {
        throw new Error(responseData.error.message || 'Failed to upload to Cloudinary');
      }

      console.log('Uploaded to Cloudinary:', responseData);

      return {
        success: true,
        data: responseData.secure_url, // Cloudinary URL
      };
    }
